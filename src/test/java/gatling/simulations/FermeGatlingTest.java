...
/**
 * Performance test for the Ferme entity.
 */
public class FermeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all fermes").get("/api/fermes").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new ferme")
                        .post("/api/fermes")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"libelle\": \"SAMPLE_TEXT\"" + ", \"photo\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_ferme_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created ferme").get("${new_ferme_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created ferme").delete("${new_ferme_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Ferme entity").exec(scn);
