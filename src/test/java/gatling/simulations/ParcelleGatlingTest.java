...
/**
 * Performance test for the Parcelle entity.
 */
public class ParcelleGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all parcelles").get("/api/parcelles").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new parcelle")
                        .post("/api/parcelles")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"libelle\": \"SAMPLE_TEXT\"" + ", \"photo\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_parcelle_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created parcelle").get("${new_parcelle_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created parcelle").delete("${new_parcelle_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Parcelle entity").exec(scn);
