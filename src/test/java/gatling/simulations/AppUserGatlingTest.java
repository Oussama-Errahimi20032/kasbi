 ...
 */
public class AppUserGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all appUsers").get("/api/app-users").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new appUser")
                        .post("/api/app-users")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created appUser").get("${new_appUser_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created appUser").delete("${new_appUser_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AppUser entity").exec(scn);
