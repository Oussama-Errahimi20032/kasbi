...
/**
 * Performance test for the Plantage entity.
 */
public class PlantageGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all plantages").get("/api/plantages").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new plantage")
                        .post("/api/plantages")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"date\": \"2020-01-01T00:00:00.000Z\"" + ", \"nombre\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_plantage_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created plantage").get("${new_plantage_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created plantage").delete("${new_plantage_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Plantage entity").exec(scn);
